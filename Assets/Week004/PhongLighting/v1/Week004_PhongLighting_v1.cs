...
[ExecuteInEditMode]
public class Week004_PhongLighting_v1 : MonoBehaviour
{
    public Light[] lights;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < lights.Length && i < MAX_COUNT; i ++)
        {
            ...
            } else if (light.type == LightType.Spot)
            {
                ...
                param.x = 1;
                param.y = Mathf.Cos(spotAngle * Mathf.Deg2Rad * 0.5f);
                param.z = Mathf.Cos(innerSpotAngle * Mathf.Deg2Rad * 0.5f);
                param.w = lightRange;
            }
        ...
        // in case unity Property (xxxxx) exceeds previous array size (3 vs 2). Cap to previous size
        for (int i = MY_LIGHT_COUNT; i < MAX_COUNT; i ++)
        {
            MY_LIGHT_POSITION[i] = Vector4.zero;
            MY_LIGHT_DIRECTIONS[i] = Vector4.zero;
            MY_LIGHT_COLORS[i] = Vector4.zero;
            MY_LIGHT_PARAMS[i] = Vector4.zero;
        }
